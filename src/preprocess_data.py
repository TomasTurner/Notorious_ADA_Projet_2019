...
def preprocess_data_file(Filename, newFilename): # for Tax and Liscence public use
    tempFilename = dataprocessed_PATH + 'tempFile.csv'
    text_to_bool(Filename, tempFilename)
    merge_str_as_header(tempFilename, newFilename)
    
def preprocess_data_file_one(Filename, newFilename): # for health_warning
    text_to_bool(Filename, newFilename)

def preprocess_public_use_data(Filename, newFilename):
    tempFilename = dataprocessed_PATH + 'tempFile.csv'
...
def preprocess_age_limits(Filename, newFilename): #for age limits
    ...
    df=df.replace({'total ban': HEURISTIC_TOTAL_BAN,
                'subnational': np.nan,
...
# function to process text data, capitalize, clear out white space and '-', especially for country name
...
# function to process country name, call def process_text
...
# replaces yes and no to values, while setting 'No data' to NaN
...
# scales the dataframe to the specified range (0,1) by default
